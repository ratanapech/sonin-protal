...
use app\modules\asset\models\search\BuildingSearch;
...
class BuildingController extends Controller
{
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['index', 'view', 'create', 'update', 'delete', 'add-floor'],
                        'roles' => ['@']
                    ],
                    [
                        'allow' => false
    ...
    public function actionView($id)
    {
        ...
        $providerFloor = new \yii\data\ArrayDataProvider([
            'allModels' => $model->floors,
        ]);
    ...
    */
    public function actionAddFloor()
    {
        if (Yii::$app->request->isAjax) {
            $row = Yii::$app->request->post('Floor');
            if((Yii::$app->request->post('isNewRecord') && Yii::$app->request->post('_action') == 'load' && empty($row)) || Yii::$app->request->post('_action') == 'add')
                $row[] = [];
            return $this->renderAjax('_formFloor', ['row' => $row]);
        } else {
            throw new NotFoundHttpException('The requested page does not exist.');
        }
    }
